...
// Fetch customer orders with order_code
...
<form method="GET" class="mb-4">
    <input type="text" name="order_code" placeholder="Search Order Code" value="<?php echo e($_GET['order_code'] ?? ''); ?>">
    <button type="submit" class="bg-blue-600 text-white px-3 py-1 rounded">Search</button>
    <a href="orders.php" class="ml-2">Clear</a>
</form>
    ...
    <div class="space-y-4">
        <?php while ($order = mysqli_fetch_assoc($result)): ?>
            <div class="bg-white border p-5 rounded shadow">
                ...
                <p><strong>Status:</strong> <?php echo e(ucfirst(str_replace('_', ' ', strtolower($order['order_status'])))); ?></p>
                ...
                <?php if (empty($order['payment_status'])): ?>
                    <form action="<?php echo BASE_URL; ?>backend/actions/make_payment.php" method="POST" class="mt-3">
                        ...
                        <button type="submit" name="pay_order" class="bg-green-600 text-white px-4 py-2 rounded">
                            Pay Now via GCash
                        </button>
                    </form>
                <?php else: ?>
